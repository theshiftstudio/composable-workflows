...
import com.shiftstudio.workflow.sample.tictactoe.gameworkflow.Ending.Quitted
import com.shiftstudio.workflow.sample.tictactoe.gameworkflow.GameLog.LogResult.LOGGED
import com.shiftstudio.workflow.sample.tictactoe.gameworkflow.GameLog.LogResult.TRY_LATER
import com.shiftstudio.workflow.sample.tictactoe.gameworkflow.RunGameResult.CanceledStart
import com.shiftstudio.workflow.sample.tictactoe.gameworkflow.RunGameResult.FinishedPlaying
import com.shiftstudio.workflow.sample.tictactoe.gameworkflow.RunGameState.*
                ...
                RunGameRendering(
                    ...
                    namePrompt = object : ScreenOverlay<Screen> {
                        override val content = NewGameScreen(
                            ...
                            onStartGame = context.eventHandler { x, o ->
                        ...
                        maybeQuitScreen(
                            ...
                            confirmQuit = context.eventHandler {
                                (state as? MaybeQuitting)?.let { oldState ->
                                    state = MaybeQuittingForSure(
                                        oldState.playerInfo,
                                        oldState.completedGame
                                    )
                                }
                            },
                            continuePlaying = context.eventHandler {
                                (state as? MaybeQuitting)?.let { oldState ->
                                    state = Playing(
                                        oldState.playerInfo,
                                        oldState.completedGame.lastTurn
                                    )
                                }
                            }
                        )
